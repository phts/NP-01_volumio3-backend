...
PlatformSpecific.prototype.fastReboot = function () {
...
PlatformSpecific.prototype.networkRestart = function () {
  var self = this
  exec(
    '/usr/bin/sudo /bin/ip addr flush dev eth0 && /usr/bin/sudo /sbin/ifconfig eth0 down && /usr/bin/sudo /sbin/ifconfig eth0 up',
    function (error, stdout, stderr) {
      if (error !== null) {
        self.coreCommand.pushToastMessage(
          'error',
          self.coreCommand.getI18nString('NETWORK.NETWORK_RESTART_TITLE'),
...
PlatformSpecific.prototype.wirelessRestart = function () {
  ...
  exec('sudo /bin/systemctl restart wireless.service', function (error, stdout, stderr) {
    if (error !== null) {
      self.coreCommand.pushToastMessage(
        'error',
        self.coreCommand.getI18nString('NETWORK.WIRELESS_RESTART_TITLE'),
