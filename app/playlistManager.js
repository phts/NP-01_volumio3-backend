...
PlaylistManager.prototype.removeFromFavourites = function (name, service, uri) {
  ...
  if (service === 'webradio') {
    ...
    return self
      .commonRemoveFromPlaylist(self.favouritesPlaylistFolder, 'favourites', service, uri)
      .then(function (data) {
        ...
        if (uri.startsWith('genres://')) {
          ...
          prms.then(function (info) {
            ...
            for (var i = 0; i < nItems; i++) {
              ...
              if (item.type == 'song') {
                var artUrl = self.commandRouter.executeOnPlugin(
                  'music_service',
                  'mpd',
                  'getAlbumArt',
                  {
...
PlaylistManager.prototype.saveJSONFile = function (localFolder, fileName, data) {
  ...
  if (
    this.commandRouter.sharedVars.get('myVolumio.cloudDeviceEnabled') === true &&
    this.commandRouter.sharedVars.get('myvolumio.uid') !== '0'
  ) {
    ...
    if (err) {
      ...
      self
        ...
        .fail(function () {
          self.commandRouter.pushToastMessage(
            'error',
            self.commandRouter.getI18nString('PLAYLIST.REMOVE_ERROR'),
            itemName
          )
...
PlaylistManager.prototype.commonAddItemsToPlaylist = function (folder, name, data) {
  ...
  self
    .saveJSONFile(folder, name, playlist)
    .then(function () {
      defer.resolve()
    })
    .fail(function () {
