...
volumioAppearance.prototype.getUIConfig = function () {
  ...
  self.commandRouter
    .i18nJson(
      __dirname + '/../../../i18n/strings_' + config.get('language_code') + '.json',
...
volumioAppearance.prototype.generateThumbnails = function () {
  ...
  return libQ
    .nfcall(fs.readdir, backgroundPath)
    .then((files) => {
      var defers = []
      var map = {}
    ...
    .fail((e) => {
      self.logger.error('Failed to generate thumbnails: ' + e)
    })
    .then(() => {
      return 'Ok'
    })
...
volumioAppearance.prototype.createThumbnailPath = function () {
  ...
  return libQ
    .nfcall(fs.mkdir, backgroundPath)
    .fail(function (e) {
      if (e.code === 'EEXIST') {
        return null
      } else {
    ...
    .then(function () {
      ...
      return dataFolder.then(function (dataFiles) {
        pluginFolder.then(function (pluginFiles) {
          if (
            pluginFiles.every(function (f) {
              return dataFiles.includes(f)
            })
          ) {
            return null
          } else {
...
volumioAppearance.prototype.setBackgrounds = function (data) {
  ...
  self.commandRouter.pushToastMessage(
    'success',
    self.commandRouter.getI18nString('APPEARANCE.APPEARANCE'),
...
volumioAppearance.prototype.setVolumio3UI = function (data) {
  ...
  this.setActiveUI(data.volumio3_ui.value)
}
...
volumioAppearance.prototype.setActiveUI = function (data) {
  ...
  if (!found) {
    ...
    self.commandRouter.pushToastMessage(
      ...
      self.commandRouter.getI18nString('APPEARANCE.FAILED_TO_SELECT_USER_INTERFACE')
    )
  }
}
...
volumioAppearance.prototype.saveActiveUIFile = function (activeUiJson) {
  ...
  if (fs.existsSync(activeUiJson.uiPath)) {
    try {
      ...
      self.commandRouter.pushToastMessage(
        'success',
        self.commandRouter.getI18nString('APPEARANCE.USER_INTERFACE_SETTINGS'),
        self.commandRouter.getI18nString('APPEARANCE.USER_INTERFACE_SUCCESSFULLY_SET_TO') +
          ' ' +
          activeUiJson.uiPrettyName
      )
    } catch (e) {
...
volumioAppearance.prototype.sendSizeErrorToasMessage = function (size) {
  ...
  return self.commandRouter.pushToastMessage(
    'error',
    self.commandRouter.getI18nString('APPEARANCE.UPLOAD_FAILED'),
    self.commandRouter.getI18nString('APPEARANCE.IMAGE_MUST_BE_LESS_THAN') + ' ' + size + ' MB'
  )
}
