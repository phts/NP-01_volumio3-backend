...
CommandLineClient.prototype.buildVolumeFiles = function () {
  ...
  var getVolumeDefer = libQ.nfcall(fs.readFile, path.join(__dirname, 'getvolume.sh.template')).fail(function (e) {
    return null
  })
  var setVolumeDefer = libQ.nfcall(fs.readFile, path.join(__dirname, 'setvolume.sh.template')).fail(function (e) {
    return null
  })
  ...
  return getVolumeDefer.then(function (getVolumeTemplate) {
    return setVolumeDefer.then(function (setVolumeTemplate) {
      if (getVolumeTemplate && getVolumeTemplate.length && setVolumeTemplate && setVolumeTemplate.length) {
        ...
        return libQ.all(self.writeVolumeFiles('/tmp/setvolume'), self.writeVolumeFiles('/tmp/getvolume', getCommand))
      }
    })
  })
}
...
CommandLineClient.prototype.writeVolumeFiles = function (path, content) {
  ...
  return libQ
    .nfcall(fs.writeFile, path, toWrite, 'utf8')
    .then(function (x) {
